...
tspan=[0,nyr];
par=pirtModelPar;par.wnpp=wnpp;par.dnpp=dnpp;
...
par=ComproModelPar;par.wnpp=wnpp;par.dnpp=dnpp;
...
par=mDroopModelPar;par.wnpp=wnpp;par.dnpp=dnpp;
...
par=VISModelPar;par.wnpp=wnpp;par.dnpp=dnpp;
...
par=sDEBModelPar;par.wnpp=wnpp;par.dnpp=dnpp;
...
par=mDEBModelPar;par.wnpp=wnpp;par.dnpp=dnpp;
...
set_curAX(fig,ax(1));
par=pirtModelPar;
hdl(1)=plot(diff(y1(:,par.vid.mu))./(t1(2)-t1(1)),1-diff(y1(:,par.vid.CO2))./diff(y1(:,par.vid.hup)),'-','LineWidth',1.5);
hold on;
hdl(2)=plot(diff(y2(:,par.vid.mu))./(t2(2)-t2(1)),1-diff(y2(:,par.vid.CO2))./diff(y2(:,par.vid.hup)),'-.','LineWidth',1.5);
...
par=mDroopModelPar;
set_curAX(fig,ax(2));
hdl(1)=plot(diff(y3(:,par.vid.mu))./(t3(2)-t3(1)),1-diff(y3(:,par.vid.CO2))./diff(y3(:,par.vid.hup)),'LineWidth',1.5);
ylabel('Total biomass CUE');
xlabel('Specific growth rate  (yr^-^1)');
...
hdl(1)=plot(diff(y4(:,par.vid.mu))./(t4(2)-t4(1)),1-diff(y4(:,par.vid.CO2))./diff(y4(:,par.vid.hup)),'LineWidth',1.5);
hold on;
hdl(2)=plot(diff(y5(:,par.vid.mu))./(t5(2)-t5(1)),1-diff(y5(:,par.vid.CO2))./diff(y5(:,par.vid.hup)),'LineWidth',1.5);
hdl(3)=plot(diff(y6(:,par.vid.mu))./(t6(2)-t6(1)),1-diff(y6(:,par.vid.CO2))./diff(y6(:,par.vid.hup)),'LineWidth',1.5);
ylabel('Total biomass CUE');
xlabel('Specific growth rate (yr^-^1)');
...
par=mDroopModelPar;
arrowPlot(fig,ax(1),diff(y3(:,par.vid.mu))./(t3(2)-t3(1)),1-diff(y3(:,par.vid.CO2))./diff(y3(:,par.vid.hup)),300);
box on;
ylabel('Total biomass CUE');
xlabel('Specific growth rate (yr^-^1)');
...
arrowPlot(fig,ax(2),diff(y6(:,par.vid.mu))./(t6(2)-t6(1)),1-diff(y6(:,par.vid.CO2))./diff(y6(:,par.vid.hup)),300);
box on;
ylabel('Total biomass CUE');
xlabel('Specific growth rate (yr^-^1)');
...
set_curAX(fig,ax(1));
set(ax,'Xlim',[0.7,1.3],'Ylim',[0.35,0.42]);
par=mDroopModelPar;
dt=(t3(2)-t3(1));
arrowPlot(fig,ax(1),diff(y3(:,par.vid.mu))./dt,diff(y3(:,par.vid.cue))./dt,200);
...
par=mDEBModelPar;
dt=(t4(2)-t4(1));
arrowPlot(fig,ax(2),diff(y4(:,par.vid.mu))./dt,diff(y4(:,par.vid.cue))./dt,200);
box on;

set_curAX(fig,ax(3));
%set(ax(3),'YLim',[.37,.382]);
dt=(t5(2)-t5(1));
arrowPlot(fig,ax(3),diff(y5(:,par.vid.mu))./dt,diff(y5(:,par.vid.cue))./dt,200);
xlabel('Specific growth rate (yr^-^1)');
ylabel('Structural biomass CUE');
box on;
...
dt=(t6(2)-t6(1));
hdl=arrowPlot(fig,ax(4),diff(y6(:,par.vid.mu))./dt,diff(y6(:,par.vid.cue))./dt,200);

box on;
set(ax,'FontSize',18);

xlabel('Specific growth rate (yr^-^1)');
put_tag(fig,ax(1),[0.025,0.93],'(a) mDroop Model',18);
put_tag(fig,ax(2),[0.025,0.93],'(b) VIS Model',18);
put_tag(fig,ax(3),[0.025,0.93],'(c) sDEB Model',18);
put_tag(fig,ax(4),[0.025,0.93],'(d) mDEB Model',18);
%%
